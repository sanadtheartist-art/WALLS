<template>
  <div class="w-full space-y-4">
    ...
    <div class="flex items-center justify-between">
      <h2 class="text-xl sm:text-2xl font-bold text-white">My Blocks</h2>
      <span class="text-sm text-gray-500">{{ authStore.blocks.length }} block{{ authStore.blocks.length !== 1 ? 's' : '' }}</span>
    </div>

    <!-- Block Type Picker -->
    <div class="bg-[#0B1120] p-4 rounded-2xl border border-[#1e293b]">
      <p class="text-xs text-gray-500 uppercase tracking-widest mb-3 font-semibold">Add a Block</p>
      <div class="grid grid-cols-4 sm:grid-cols-5 gap-2">
        <button v-for="bt in blockTypes" :key="bt.type" @click="addBlock(bt.type)"
          class="flex flex-col items-center gap-1 p-2 rounded-xl bg-[#111827] hover:bg-[#1e293b] border border-[#1e293b] hover:border-orange-500/40 transition-all group">
          <Icon :icon="bt.icon" class="w-5 h-5" />
          <span class="text-[10px] sm:text-xs text-gray-400 group-hover:text-white font-medium">{{ bt.label }}</span>
        </button>
      </div>
    </div>
    ...
    <div v-else-if="authStore.blocks.length === 0" class="text-center py-12 bg-[#0B1120] rounded-2xl border border-dashed border-[#1e293b]">
      <Icon icon="lucide:layout-template" class="w-10 h-10 mb-3 mx-auto text-gray-600" />
      <h3 class="text-base sm:text-lg font-bold text-white mb-2">Your wall is empty</h3>
    ...
    <div v-else class="space-y-3">
      <div v-for="(block, index) in authStore.blocks" :key="block.id"
        ...
        <!-- Block Header Row -->
        <div class="flex items-center gap-3 px-4 py-3 cursor-pointer" @click="toggleExpand(block.id)">
          <!-- Drag Handle -->
        ...
        <div v-if="expandedBlock === block.id" class="px-4 pb-4 border-t border-[#1e293b] pt-4">
